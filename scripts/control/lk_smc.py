...
class lk_smc_param:
    def __init__(self, name: str="~lk_smc_parameters"):
        _p = rospy.get_param(name)
        self.dim = int(_p['dim'])
        self.dt = _p['dt']
        self.eta = np.array(_p['eta']).astype(float)
        self.gamma2 = np.array(_p['gamma2']).astype(float)
        self.gamma3 = np.array(_p['gamma3']).astype(float)
        self.Ts = np.array(_p['Ts']).astype(float)
        self.Tu = np.array(_p['Tu']).astype(float)
        ...
        self.w0 = np.array(_p['w0']).astype(float)
