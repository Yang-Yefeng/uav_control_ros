...
def plot_pos():
    ...
    plt.grid(True)
    y_min = np.min(np.concatenate((uav_pos[:, 0], ref_pos[:, 0])))
    y_max = np.max(np.concatenate((uav_pos[:, 0], ref_pos[:, 0])))
    ...
    plt.grid(True)
    y_min = np.min(np.concatenate((uav_pos[:, 1], ref_pos[:, 1])))
    y_max = np.max(np.concatenate((uav_pos[:, 1], ref_pos[:, 1])))
    plt.ylim((round(y_min - 1), round(y_max + 1)))
    # plt.yticks(np.arange(-2, 2, 0.5))
    plt.xlabel('time(s)')
    plt.title('Y')
    ...
    plt.grid(True)
    y_min = np.min(np.concatenate((uav_pos[:, 2], ref_pos[:, 2])))
    y_max = np.max(np.concatenate((uav_pos[:, 2], ref_pos[:, 2])))
    plt.ylim((round(y_min - 1), round(y_max + 1)))
